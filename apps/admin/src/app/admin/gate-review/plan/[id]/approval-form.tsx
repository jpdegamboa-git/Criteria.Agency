...
interface GateApprovalFormProps {
  planId: string;
}

export function GateApprovalForm({ planId }: GateApprovalFormProps) {
  ...
  async function handleAction(action: 'approve' | 'reject') {
    ...
    try {
      const res = await fetch(`/api/strategist/plans/${planId}/${action}`, {
        method: 'PUT',
        credentials: 'include',
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <h2 className="text-sm font-semibold text-gray-700 mb-4">Decisión G3</h2>
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          placeholder="Feedback para el equipo..."
        />
      </div>
      ...
      <div className="flex gap-3">
        ...
        >
          {loading === 'approve' ? 'Aprobando...' : '✓ Aprobar Plan'}
        </button>
        <button
        ...
        >
          {loading === 'reject' ? 'Rechazando...' : '✗ Rechazar Plan'}
        </button>
      </div>
    </div>
  );
