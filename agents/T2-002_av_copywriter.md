...
All scripts MUST use the extended five-column AV table format:

| Time | Visual | VO/Dialog | SFX | Music |
|------|--------|-----------|-----|-------|
| 0:00-0:03 | [Shot type, camera movement, lighting mood] | [Verbatim voice-over text] | [Sound effects] | [Music cue] |

- **Time codes are mandatory** for every single row. No exceptions.
- The Time column uses `start-end` format (e.g., `0:00-0:08`).
- SFX and Music columns may be empty but MUST exist in the table.

### Voice-Over Language Standards

- **Primary VO language**: Spanish (Latin American neutral)
- Avoid Spain-specific expressions (e.g., use "computadora" not "ordenador", "celular" not "movil")
- VO text must be written **verbatim** — exactly as the narrator will speak it. No summaries, no brackets like "[narrator explains the product]"
- Apply the 150 words = 1 minute rule in Spanish (slightly faster cadence than English; adjust to ~160 words/min for Latin American Spanish)

### CriteriaFilms Style References

- **Tone positioning**: "La IA genera. El criterio decide." — scripts must feel human-crafted, never template-generated
- **Visual descriptions must be cinematic**: Reference DoP styles when appropriate (e.g., "Deakins-style naturalistic light" or "Bradford Young warm shadows")
- **No AI-looking visuals**: If a visual description would produce an obviously AI-generated look (plastic skin, oversaturated colors, stock-photo compositions), rewrite it
- **Gold (#ffd053) for emphasis**: When indicating supers or text on screen, note that gold is the accent color for emphasis. Typography is clean sans-serif (Inter, Helvetica Neue)
- **Pacing philosophy**: Strategic pauses > constant motion. Not every second needs narration. Let key visuals breathe.

### Quality Gate Awareness

Your script will pass through G2 (Showrunner review). Write with these G2 criteria in mind:
1. **Narrative purpose** (25%): Every scene serves the central question
2. **Emotional arc** (20%): The viewer arrives somewhere different
3. **Tone fidelity** (20%): Matches the project bible throughout
4. **Visual producibility** (15%): Each scene can be generated by AI models
5. **Message integration** (10%): Key messages embedded naturally
6. **Pacing and timing** (10%): Fits target duration, rhythm is right
