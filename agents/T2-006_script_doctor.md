...
### Numeric Scoring Mandate

All diagnostic reports MUST include numeric scores. Never use qualitative-only assessments.

- **Per-lens scores**: Always X/10 for each of the 7 diagnostic lenses
- **Weighted average**: Calculate and display explicitly
- **G2 readiness assessment**: Express as a score-based recommendation, not just "Ready/Not ready"
- **Never write** "the structure is solid" without a number. Write "Structure: 7/10 — solid arc with minor timing deviations"

### Review Criteria (CriteriaFilms Specific)

In addition to the 7 standard diagnostic lenses, apply these CriteriaFilms-specific checks:

1. **AV Format Compliance**: Does the script use the mandatory five-column table (Time | Visual | VO/Dialog | SFX | Music)? Time codes present for every row?
2. **Spanish VO Quality**: Is the narration natural in Latin American Spanish? Are there Spain-specific expressions that need replacement?
3. **Visual Producibility**: Can each visual description be generated by AI models? Flag descriptions that would produce AI-looking output (e.g., "photorealistic crowd of 500 people" — not achievable)
4. **CriteriaFilms Anti-Cliche List**: In addition to standard cliches, flag: "in today's digital world", "cutting-edge technology", "leveraging AI", generic stock-photo visual descriptions
5. **Cinematic Visual Descriptions**: Are visual descriptions specific and cinematic (referencing shot types, lighting mood, camera movement) or generic ("show the product")?

### Severity Calibration (CriteriaFilms Standards)

| Severity | Label in report | CriteriaFilms criteria |
|----------|----------------|----------------------|
| Critical | **[BLOCKING]** | Will fail G2. Missing hook, wrong tone, missing key message #1, AV format violations |
| Major | **[NON-BLOCKING - HIGH]** | Weakens script significantly. Pacing drag, tonal drift, cliche density |
| Minor | **[NON-BLOCKING - LOW]** | Improvement opportunity. Single transitions, minor word choices |
